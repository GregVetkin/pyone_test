...
def test_flatten_not_active_image_snapshot(prepare_image_with_5_snapshots):
    one                 = One(BRESTADM_SESSION)
    ...
    one.image.snapshotflatten(image._id, snap_id)
    
    assert image.info().SNAPSHOTS == []




def test_flatten_active_image_snapshot(prepare_image_with_5_snapshots):
    one             = One(BRESTADM_SESSION)
    image           = prepare_image_with_5_snapshots
    active_snap_id  = next(snapshot.ID for snapshot in image.info().SNAPSHOTS if snapshot.ACTIVE)

    one.image.snapshotflatten(image._id, active_snap_id)

...
def test_vm_not_exist(one: One, replace: bool):
    vm_id = random.randint(9999, 999999)
    template = ""

    with pytest.raises(OneNoExistsException):
        one.vm.update(vm_id, template, replace)
...
@pytest.mark.parametrize("replace", [True, False])
def test_update_and_replace(one: One, dummy_vm: int, replace: bool):
    vm_id = dummy_vm
    ...
    one.vm.update(vm_id, start_template,False)
    ...
    _id = one.vm.update(vm_id, attr_template, replace)
    assert _id == vm_id
    vm_user_template = one.vm.info(vm_id, True).USER_TEMPLATE

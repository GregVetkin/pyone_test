...
@pytest.mark.parametrize("one", [ADMIN_NAME], indirect=True)
def test_rename_snapshot(one: One, vm_with_disk_snapshots: VirtualMachine):
    vm_id                = vm_with_disk_snapshots._id
    new_snapshot_name    = get_unic_name()
    disks_snapshot_names = {disk_snapshots_info.DISK_ID : {snapshot_info.ID: snapshot_info.NAME for snapshot_info in disk_snapshots_info.SNAPSHOT}
                            for disk_snapshots_info in one.vm.info(vm_id).SNAPSHOTS}

    while True:
        target_disk_id     = random.choice(list(disks_snapshot_names.keys()))
        target_snapshot_id = random.choice(list(disks_snapshot_names[target_disk_id].keys()))
    ...
    assert new_snapshot_name == disks_snapshot_names_after[target_disk_id][target_snapshot_id]

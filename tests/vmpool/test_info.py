...
def test_all_vms(one: One, vms: List[int]):
    ...
    vmpool = one.vmpool.info(filter_flag, start_id, end_id, vm_state_filter, key_value_filter).VM
...
def test_state_filter_except_done(one: One, vms: List[int]):
    filter_flag      = -2
    start_id         = -1
    end_id           = -1
...
def test_range_id_filter(one: One, vms: List[int]):
    vm_ids = vms
